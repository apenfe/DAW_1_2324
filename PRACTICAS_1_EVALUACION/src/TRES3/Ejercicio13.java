...
public class Ejercicio13{
    ...
    public static void main(String[] args){
        ...
        System.out.print("\nA continuación inserte una palabra: ");
        String palabra = teclado.nextLine().trim().toUpperCase();
        palabra.replaceAll("[^A-Za-z]","");
        ...
        for(int i = 0; i<palabra.length(); i++){
            ...
            letras[i]=bannerLetra(letra);
        }

        System.out.println();
        System.out.println(bannerTexto(letras));
    }

    public static String bannerTexto(String[] letras){

        String texto="";
        int cont=0;

        for(int i=1; i<=7 ;i++){ //fila

            for(int j=1; j<=letras.length; j++){ //letra de la palabra

                for(int k=cont; k<cont+7 ; k++){ // parte letra 

                    texto+=letras[j-1].charAt(k);
                }
