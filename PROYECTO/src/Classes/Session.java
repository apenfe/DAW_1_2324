...
public class Session{
	...
	//private final String FILE_PATH = "./assets/files/"; // EN LINUX ./ AL INICIO Y EN EL RESTO DE LA RUTA /
	...
	public void login() {
		...
		int count=1; // CONTADOR PARA DAR VARIAS OPORTUNIDADES DE INICIO DE SESION ANTES DE VOLVER AL MENU
		...
		do {
			...
			if(trowQuery(userdata,FILE_PATH+USER_FILE,"#","login")) { // BUSCA UNA COOINCIDENCIA EN EL FICHERO
				break; // SI LA ENCUENTRA TERMINA
			}else {
				count++; // SI NO LA ENCUENTRA RESTA UN INTENTO
				System.out.println("\t\tNombre de usuario y/o contraseña incorrectos.");
				System.out.println("\t\tLe quedan "+(4-count)+" intentos.");
			}
				
		}while(count<=3);
	
		if(count==4) {
			System.out.println("\n\t\tPuede que aún no esté registrado.");
		}
	}
	...
	private void createUser(String file, String symbol, String[] data) {
		...
		/* ESTE METODO RECIBE UN ARRAY CON TODOS LOS DATOS CORRECTOS DEL USUARIO */
		 ...
		 try {
	            ...
	            dataIn.write(data[0]+symbol+data[1]+symbol+data[2]+symbol+data[3]+symbol+data[4]+symbol+data[5]+symbol+data[6]+symbol+"user");
			    dataIn.newLine();
			    dataIn.close();
			...
			try {
				...
				while(query.hasNextLine()) { // MIENTRAS QUEDE UNA NUEVA LINEA EN EL FICHERO ABIERTO...
					...
					if(option.equals("login")) { /* SI HEMOS ELEGIDO LA OPCION LOGIN */
						...
						if(data[0].equals(userdata[0]) || data[3].equals(userdata[3]) || data[4].equals(userdata[4]) ) { // SI HAY ALGUNA COINCIDENCIA DE PK
							
							/* SE INDICAN LAS COINCIDENCIAS */
							System.out.println("\n\t\tRegistro incorrecto (ERRORES):");
							
							if(data[0].equals(userdata[0])) {
								System.out.println("\t\t- Nombre de usuario en uso.");
							}
							
							if(data[3].equals(userdata[3])) {
								System.out.println("\t\t- NIF en uso. ");
							}
							
							if(data[4].equals(userdata[4])) {
	...
	private boolean primaryKey(String[] userdata) {
			...
			do { /* MIENTRAS NO SEAN DATOS VALIDOS O EL USUARIO PERSISTA */
				...
				int cont=3; // UTILIZADO PARA SABER SI HAY ALGUN ERROR
				
				userdata[0] = Input.getString("\n\n\tIntroduzca un nombre de usuario: ");
				
				if(!Config.checkMaxLong(userdata[0],20)||!Config.checkSymbols(userdata[0])||!Config.checkName(userdata[0])) {
					cont --;
				...
				if(cont==3) { // SI NO HAY ERROR DEVUELVE TRUE
					
					return true;
					
				}else if(cont<3) { // SI HAY ALGUN ERROR PREGUNTA SI DESEA SEGUIR
	...
	private boolean secondaryKey(String[] userdata) {
		...
		do { /* MIENTRAS NO SEAN DATOS VALIDOS O EL USUARIO PERSISTA */
			...
			int cont =4; // UTILIZADO PARA SABER SI HAY ALGUN ERROR
			...
			if(cont==4) { // SI NO HAY ERROR DEVUELVE TRUE
				
				System.out.println("\n\tCreando usuario, por favor espere...");
				return true;
				
			}else if(cont<4) { // SI HAY ALGUN ERROR PREGUNTA SI DESEA SEGUIR
